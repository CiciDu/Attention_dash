...
class DashSimulation(simulation_class.Simulation):
    ...
    def generate_simulation_params(self):
        self.simulation_params = {'num_trial': self.num_trial,
                                ...
                                'high_attn_ts_per_trial': self.high_attn_ts_per_trial,
    ...
    def generate_input_items(self):
        ...
        self.input_items = {'count': [['num_trial', 'Trial count', 1, 1000000],
                                    ...
                                    ['high_attn_ts_per_trial', 'High-attention time steps per trial', 1, 'ts_per_trial'],
    ...
    def prepare_dash_for_main_plots_layout(self, id_prefix=''):
        ...
        return html.Div([
                        ...
                        dash_simul_helper_func.put_down_run_simulation_button(),
                        dash_simul_helper_func.put_down_y_axis_variable(self.y, self.n_combo_to_plot, self.highest_or_lowest),
                        dash_simul_helper_func.put_down_refresh_plot_button(),
                        dash_simul_helper_func.put_down_main_plot(self.fig),
                        ])
    ...
    def make_function_to_refresh_plot(self, app):
        @app.callback(
            ...
            State("y_var", "value"),
            State("n_combo_to_plot", "value"),
            State("highest_or_lowest", "value"),
            prevent_initial_call=True
        )
        def refresh_plot(n_clicks, y_var, n_combo_to_plot, highest_or_lowest):
            self.y = 'success_rate_ranking' if y_var == 'ranking' else 'combo_success_rate'
