from methods.plot_methods import plt_simulation, plt_shared
...
def add_to_base_plot(fig, x, y, ts_to_plot, ts_per_trial, n_combo_to_plot, highest_or_lowest):
    fig = add_high_attention_points(fig, x, y, ts_to_plot)
    label_dict = plt_shared.generate_label_dict()
    fig = add_labels_and_title(fig, x, y, label_dict, n_combo_to_plot, highest_or_lowest, ts_per_trial)
    if y=='ranking':
        fig = label_with_success_rate(fig, ts_per_trial, ts_to_plot, highest_or_lowest)
    fig = update_axis(fig, y, ts_to_plot, highest_or_lowest)
    return fig 
...
def label_with_success_rate(fig, ts_per_trial, ts_to_plot, highest_or_lowest):
    ts_to_plot = ts_to_plot.copy()
    ts_to_plot = ts_to_plot[['ranking', 'success_rate']]
...
def add_labels_and_title(fig, x, y, label_dict, n_combo_to_plot, highest_or_lowest, ts_per_trial):
    fig.update_layout(title='Top {} high-attention timestep combinations'.format(n_combo_to_plot) if highest_or_lowest == 'highest' else 'Bottom {} high-attention timestep combinations'.format(n_combo_to_plot), 
                      title_x=0.5,
...
def update_axis(fig, y, ts_to_plot, highest_or_lowest):
    if y=='success_rate':
