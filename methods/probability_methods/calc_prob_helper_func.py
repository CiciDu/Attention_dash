...
def calc_prob_after_adding_to_existing_high_attn_ts(high_attn_ts=[],
    ...
    # make sure that adding_ts_result_df contains information of all ts
    all_ts = np.arange(1, ts_per_trial+1)
    adding_ts_result_df = adding_ts_result_df.merge(pd.DataFrame(all_ts, columns=['ts']), on='ts', how='right')
    adding_ts_result_df.sort_values(by='ts', inplace=True)
